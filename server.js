...
function sellerOrAdminRequired(req, res, next) {
  if (req.user?.role !== "admin" && req.user?.role !== "seller") {
    return res.status(403).json({ error: "Bạn chưa được cấp quyền người bán" });
  }
  next();
}
...
app.post("/api/admin/products", authRequired, adminRequired, (req, res) => {
  ...
  data.products.push({ id, name, price: Number(price), imageUrl: imageUrl || "", description: description || "", stock, ownerId: null });
  db.save(data);
  res.json({ ok: true, id });
});
...
// ================= SELLER: người bán tự quản lý sản phẩm của mình =================
app.get("/api/seller/products", authRequired, sellerOrAdminRequired, (req, res) => {
  const data = db.load();
  const mine = data.products.filter(p => p.ownerId === req.user.id);
  res.json(mine);
});

app.post("/api/seller/products", authRequired, sellerOrAdminRequired, (req, res) => {
  const { name, price, imageUrl, description, stockText } = req.body || {};
  if (!name || !price) return res.status(400).json({ error: "Thiếu tên hoặc giá sản phẩm" });
  const data = db.load();
  const id = db.nextId(data);
  const stock = (stockText || "").split("\n").map(s => s.trim()).filter(Boolean);
  data.products.push({ id, name, price: Number(price), imageUrl: imageUrl || "", description: description || "", stock, ownerId: req.user.id });
  db.save(data);
  res.json({ ok: true, id });
});

app.put("/api/seller/products/:id", authRequired, sellerOrAdminRequired, (req, res) => {
  const id = Number(req.params.id);
  const data = db.load();
  const p = data.products.find(x => x.id === id);
  if (!p) return res.status(404).json({ error: "Không tìm thấy sản phẩm" });
  if (p.ownerId !== req.user.id && req.user.role !== "admin") {
    return res.status(403).json({ error: "Đây không phải sản phẩm của bạn" });
  }
  const { name, price, imageUrl, description, stockText } = req.body || {};
  if (name !== undefined) p.name = name;
  if (price !== undefined) p.price = Number(price);
...
app.delete("/api/seller/products/:id", authRequired, sellerOrAdminRequired, (req, res) => {
  const id = Number(req.params.id);
  const data = db.load();
  const p = data.products.find(x => x.id === id);
  if (!p) return res.status(404).json({ error: "Không tìm thấy sản phẩm" });
