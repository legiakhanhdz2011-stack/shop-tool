...
// Nếu chạy sau proxy của Render/Cloudflare thì cần dòng này để rate-limit
// tính đúng theo IP thật của người dùng thay vì IP của proxy
app.set("trust proxy", 1);

app.use(cors());

// ---------- Chống DoS / flood request ----------
// Giới hạn chung cho toàn bộ /api/*: tối đa 100 request / phút / IP
...
// Giới hạn chặt hơn cho đăng nhập/đăng ký để chống brute-force + spam tài khoản
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "Thử quá nhiều lần, vui lòng thử lại sau 15 phút" }
});

// Giới hạn riêng cho webhook ngân hàng (không nên cho phép flood endpoint này)
...
app.use(express.json({ limit: "10mb" })); // nâng giới hạn để nhận được ảnh sản phẩm dạng base64
...
// 3 loại thời hạn key cố định mà mỗi sản phẩm có thể bán (admin có thể để trống loại nào không bán)
const VARIANT_DEFS = [
  { key: "thang", label: "Key Tháng" },
  { key: "quy", label: "Key Quý" },
  { key: "nam", label: "Key Năm" }
];

// Tính giá thực tế cho 1 biến thể (key tháng/quý/năm) tuỳ theo người mua có phải seller không (có giá sỉ riêng)
function effectiveVariantPrice(variant, role) {
  if (isSellerRole(role) && variant.wholesalePrice) return variant.wholesalePrice;
  return variant.price;
}

// Dữ liệu sản phẩm cũ (trước khi có nhiều loại key) chỉ có price/wholesalePrice/stock ở cấp sản phẩm.
// Hàm này tự động chuyển chúng thành 1 biến thể "Mặc định" để không mất dữ liệu / không lỗi khi nâng cấp.
function migrateProductVariants(data) {
  let changed = false;
...
// ---------- Khởi tạo tài khoản admin đầu tiên nếu chưa có + chuyển dữ liệu sản phẩm cũ sang dạng nhiều loại key ----------
(function ensureAdminAndMigrate() {
  const data = db.load();
  let changed = false;
  ...
  if (migrateProductVariants(data)) {
    changed = true;
    console.log("✅ Đã chuyển dữ liệu sản phẩm cũ sang định dạng nhiều loại key (Tháng/Quý/Năm)");
  }
...
app.get("/api/products", authOptional, (req, res) => {
  const data = db.load();
  // Luôn lấy role MỚI NHẤT từ database, không dùng role cũ nằm trong token
  // (token được cấp lúc đăng nhập nên nếu admin cấp quyền Người bán SAU khi user
  // đã đăng nhập từ trước, token cũ vẫn mang role cũ -> giá sỉ sẽ không hiện ra
  // cho tới khi họ đăng xuất/đăng nhập lại nếu không có đoạn tra cứu lại này).
  const currentUser = req.user ? data.users.find(u => u.id === req.user.id) : null;
  const role = currentUser?.role;
  const list = data.products.map(p => {
    const variants = (p.variants || []).map(v => {
      const price = effectiveVariantPrice(v, role);
      return {
        key: v.key,
        label: v.label,
        price,
        isWholesale: price !== v.price,
        remaining: (v.stock || []).length,
    ...
    return {
      ...
      variants,
      remaining: variants.reduce((s, v) => s + v.remaining, 0),
      soldCount: variants.reduce((s, v) => s + v.soldCount, 0)
    };
  });
  res.json(list);
});
...
// Chuẩn hoá payload variants gửi từ admin (mỗi loại key: price, wholesalePrice, stockText) -> mảng lưu trong DB
// Loại key nào không nhập giá (price trống/0) thì bỏ qua, coi như sản phẩm không bán loại đó.
function buildVariantsFromPayload(rawVariants, existingVariants) {
  const existingByKey = Object.fromEntries((existingVariants || []).map(v => [v.key, v]));
  const result = [];
  for (const def of VARIANT_DEFS) {
    const raw = (rawVariants || []).find(v => v.key === def.key);
    if (!raw || !raw.price) continue; // không bán loại key này
    const prev = existingByKey[def.key];
    result.push({
      key: def.key,
      label: def.label,
      price: Number(raw.price),
      wholesalePrice: raw.wholesalePrice ? Number(raw.wholesalePrice) : null,
      stock: (raw.stockText || "").split("\n").map(s => s.trim()).filter(Boolean),
      soldCount: prev ? (prev.soldCount || 0) : 0
    });
  }
  return result;
}
...
app.post("/api/admin/products", authRequired, adminRequired, (req, res) => {
  const { name, imageUrl, description, variants } = req.body || {};
  if (!name) return res.status(400).json({ error: "Thiếu tên sản phẩm" });
  const cleanVariants = buildVariantsFromPayload(variants, []);
  if (!cleanVariants.length) return res.status(400).json({ error: "Cần nhập giá cho ít nhất 1 loại key (Tháng/Quý/Năm)" });
  const data = db.load();
  const id = db.nextId(data);
  data.products.push({
    id, name, imageUrl: imageUrl || "", description: description || "",
...
app.put("/api/admin/products/:id", authRequired, adminRequired, (req, res) => {
  ...
  if (variants !== undefined) {
    const cleanVariants = buildVariantsFromPayload(variants, p.variants);
    if (!cleanVariants.length) return res.status(400).json({ error: "Cần nhập giá cho ít nhất 1 loại key (Tháng/Quý/Năm)" });
    p.variants = cleanVariants;
  }
  db.save(data);
  res.json({ ok: true });
});
...
app.post("/api/wallet/deposit", authRequired, (req, res) => {
  const { amount, forProductId, forVariantKey, forQuantity } = req.body || {};
  ...
  const deposit = {
    ...
    forVariantKey: forVariantKey || null,
...
app.post("/api/purchase", authRequired, (req, res) => {
  ...
  if (!variant) return res.status(400).json({ error: "Vui lòng chọn loại key (Tháng/Quý/Năm) trước khi mua" });
  if (!variant.stock || variant.stock.length < qty) {
    return res.status(400).json({ error: `Kho chỉ còn ${variant.stock ? variant.stock.length : 0} sản phẩm, không đủ số lượng bạn chọn` });
  }
  const unitPrice = effectiveVariantPrice(variant, user.role);
  const totalPrice = unitPrice * qty;
  if (user.balance < totalPrice) {
    return res.status(402).json({ error: "Số dư không đủ, vui lòng nạp thêm tiền", needAmount: totalPrice - user.balance });
  }
  user.balance -= totalPrice;
  const deliveredItems = variant.stock.splice(0, qty);
  variant.soldCount = (variant.soldCount || 0) + qty;
  const orderId = db.nextId(data);
  const order = {
    id: orderId, userId: user.id, productId: product.id,
    productName: product.name, variantKey: variant.key, variantLabel: variant.label,
    price: totalPrice, unitPrice, quantity: qty,
...
function applyDeposit(data, dep) {
  ...
  if (dep.forProductId) {
    ...
    const variant = product ? (product.variants || []).find(v => v.key === dep.forVariantKey) : null;
    const qty = Math.max(1, dep.forQuantity || 1);
    const unitPrice = variant ? effectiveVariantPrice(variant, user.role) : null;
    const totalPrice = unitPrice !== null ? unitPrice * qty : null;
    if (product && variant && variant.stock && variant.stock.length >= qty && totalPrice !== null && user.balance >= totalPrice) {
      user.balance -= totalPrice;
      const deliveredItems = variant.stock.splice(0, qty);
      variant.soldCount = (variant.soldCount || 0) + qty;
      const orderId = db.nextId(data);
