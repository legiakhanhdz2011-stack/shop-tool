...
app.get("/api/products", authOptional, (req, res) => {
  ...
  const list = data.products.map(p => {
    ...
    return {
      ...
      imageUrl: p.imageUrl, description: p.description, remaining: (p.stock || []).length
    };
  });
  res.json(list);
});
...
app.get("/api/wallet/deposit/:id/status", authRequired, (req, res) => {
  ...
  res.json({ status: dep.status, deliveredItem: deliveredOrder ? deliveredOrder.deliveredItem : null });
});
...
app.post("/api/purchase", authRequired, (req, res) => {
  ...
  const price = effectivePrice(product, user.role);
  if (user.balance < price) {
    return res.status(402).json({ error: "Số dư không đủ, vui lòng nạp thêm tiền", needAmount: price - user.balance });
  }
  user.balance -= price;
...
function applyDeposit(data, dep) {
  ...
  if (dep.forProductId) {
    ...
    const price = product ? effectivePrice(product, user.role) : null;
    if (product && product.stock && product.stock.length > 0 && price !== null && user.balance >= price) {
      user.balance -= price;
      const item = product.stock.shift();
      const orderId = db.nextId(data);
      const order = {
