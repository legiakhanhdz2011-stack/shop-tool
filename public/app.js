...
let pendingHistoryRedirect = false;
function closeModal(id) {
  $("#" + id).classList.add("hidden");
  // Sau khi khách đóng popup "Mua hàng thành công", tự động mở Lịch sử để họ thấy đơn vừa mua
  if (id === "successModal" && pendingHistoryRedirect) {
    pendingHistoryRedirect = false;
    setTimeout(() => openHistory(), 150);
  }
}
...
function showSuccessModal({ title, subtitle, deliveredItems, redirectToHistory }) {
  pendingHistoryRedirect = !!redirectToHistory;
...
function renderProductCard(p) {
  ...
  const firstAvailable = variants.find(v => v.remaining > 0) || variants[0] || null;
  const anyStock = variants.some(v => v.remaining > 0);
  const variantsHtml = variants.length ? `
    <div class="variant-row">
      ${variants.map(v => `
        <button type="button"
          class="variant-chip ${firstAvailable && v.key === firstAvailable.key ? "active" : ""} ${v.remaining <= 0 ? "disabled" : ""}"
          data-variant-btn="${p.id}" data-variant-key="${v.key}"
          data-price="${v.price}" data-remaining="${v.remaining}"
          ${v.remaining <= 0 ? "disabled" : ""}>
          <span class="variant-label">${v.label}${v.isWholesale ? " 🏷️" : ""}</span>
          <span class="variant-price">${Number(v.price).toLocaleString("vi-VN")}đ</span>
        </button>
      `).join("")}
    </div>
  ` : `<p class="hint">Sản phẩm chưa có loại key nào</p>`;
  ...
  return `
    <div class="card" data-product-card="${p.id}">
      ...
      ${variantsHtml}
      <div class="stock-row">
        <span class="stock" id="stockText-${p.id}">${firstAvailable ? (firstAvailable.remaining > 0 ? "Còn " + firstAvailable.remaining : "Hết hàng") : "Hết hàng"}</span>
      ...
      ${anyStock ? `
        <div class="qty-row">
          ...
          <div class="qty-control">
            ...
            <input type="number" class="qty-input" id="qty-${p.id}" value="1" min="1" max="${firstAvailable ? firstAvailable.remaining : 1}" />
            <button type="button" class="qty-btn" data-qty-inc="${p.id}">+</button>
          </div>
        </div>
        <button class="btn btn-primary full" data-buy="${p.id}">Mua ngay</button>
      ` : `
...
async function loadProducts() {
  ...
  document.querySelectorAll("[data-variant-btn]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const pid = btn.dataset.variantBtn;
      document.querySelectorAll(`[data-variant-btn="${pid}"]`).forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      const remaining = Number(btn.dataset.remaining);
      const qtyInput = document.getElementById("qty-" + pid);
      if (qtyInput) {
        qtyInput.max = remaining;
        if (Number(qtyInput.value) > remaining) qtyInput.value = Math.max(1, remaining);
      }
      const stockEl = document.getElementById("stockText-" + pid);
      if (stockEl) stockEl.textContent = remaining > 0 ? "Còn " + remaining : "Hết hàng";
    });
  });
  ...
  document.querySelectorAll("[data-buy]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const activeVariantBtn = document.querySelector(`[data-variant-btn="${productId}"].active`);
      if (!activeVariantBtn) { toast("Vui lòng chọn loại key (Tháng/Quý/Năm)"); return; }
      const variantKey = activeVariantBtn.dataset.variantKey;
      const unitPrice = Number(activeVariantBtn.dataset.price);
      const variantLabel = activeVariantBtn.querySelector(".variant-label").textContent;
      const qtyInput = document.getElementById("qty-" + productId);
      const qty = qtyInput ? Math.max(1, Number(qtyInput.value) || 1) : 1;
      const product = productsCache.find(p => p.id === productId);
      openPurchaseConfirm({
        productId, variantKey, unitPrice, qty,
...
// ---------- Xác nhận mua hàng (2 bước, chống bấm nhầm) ----------
let pendingPurchase = null;

function openPurchaseConfirm({ productId, variantKey, unitPrice, qty, productName, variantLabel }) {
  pendingPurchase = { productId, variantKey, unitPrice, qty };
  $("#pcProductName").textContent = productName;
  $("#pcVariantLabel").textContent = variantLabel;
  $("#pcQty").textContent = qty;
  $("#pcTotal").textContent = Number(unitPrice * qty).toLocaleString("vi-VN") + "đ";
  $("#pcConfirmSummary").textContent = `${qty} × ${productName} (${variantLabel})`;
  $("#pcStep1").classList.remove("hidden");
  $("#pcStep2").classList.add("hidden");
  openModal("purchaseConfirmModal");
}

$("#pcContinueBtn").addEventListener("click", () => {
  $("#pcStep1").classList.add("hidden");
  $("#pcStep2").classList.remove("hidden");
});
$("#pcBackBtn").addEventListener("click", () => {
  $("#pcStep2").classList.add("hidden");
  $("#pcStep1").classList.remove("hidden");
});
$("#pcConfirmBtn").addEventListener("click", async () => {
  if (!pendingPurchase) return;
  const p = pendingPurchase;
  pendingPurchase = null;
  closeModal("purchaseConfirmModal");
  await buyProduct(p.productId, p.variantKey, p.unitPrice, p.qty);
});

async function buyProduct(productId, variantKey, unitPrice, quantity) {
  if (!token) { openModal("authModal"); return; }
  try {
    const data = await api("/api/purchase", { method: "POST", body: JSON.stringify({ productId, variantKey, quantity }) });
    me.balance -= data.order.price;
    updateHeader();
    loadProducts();
    showSuccessModal({
      title: quantity > 1 ? `🎉 Mua ${quantity} sản phẩm thành công!` : "🎉 Mua hàng thành công!",
      subtitle: "Cảm ơn bạn đã ủng hộ shop. Đơn hàng đã được lưu vào Lịch sử.",
    ...
    if (err.message.includes("Số dư không đủ")) {
      ...
      openDepositForProduct(productId, variantKey, unitPrice * quantity, quantity);
    } else {
      toast("❌ " + err.message);
    }
  }
...
let currentForVariantKey = null;
let currentForQuantity = 1;

els.btnDeposit.addEventListener("click", () => {
  currentForProductId = null;
  currentForVariantKey = null;
  currentForQuantity = 1;
  resetDepositModal();
  openModal("depositModal");
});

function openDepositForProduct(productId, variantKey, totalAmount, quantity) {
  currentForProductId = productId;
  currentForVariantKey = variantKey;
...
$("#btnCreateDeposit").addEventListener("click", async () => {
  ...
  try {
    const data = await api("/api/wallet/deposit", {
      method: "POST",
      body: JSON.stringify({ amount, forProductId: currentForProductId, forVariantKey: currentForVariantKey, forQuantity: currentForQuantity })
    });
      ...
      if (s.status === "completed") {
        ...
        if (s.deliveredItems && s.deliveredItems.length) {
          showSuccessModal({
            ...
            subtitle: "Hệ thống đã tự động giao sản phẩm cho bạn. Đơn hàng đã được lưu vào Lịch sử.",
